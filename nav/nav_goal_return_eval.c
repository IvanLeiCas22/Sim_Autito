...
typedef struct NavGoalReturnWorkspace {
    uint16_t cost[NAV_GOAL_RETURN_MAX_CELLS];
    uint8_t closed[NAV_GOAL_RETURN_MAX_CELLS];
    int16_t parent[NAV_GOAL_RETURN_MAX_CELLS];
    uint8_t parent_dir[NAV_GOAL_RETURN_MAX_CELLS];
    uint8_t unknown_cells[NAV_GOAL_RETURN_MAX_CELLS];
    uint8_t unknown_edges[NAV_GOAL_RETURN_MAX_CELLS];
    uint16_t reverse_path[NAV_GOAL_RETURN_MAX_CELLS];
    uint16_t path[NAV_GOAL_RETURN_MAX_CELLS];
...
{
    for (uint16_t i = 0u; i < NAV_GOAL_RETURN_MAX_CELLS; ++i) {
...
static int16_t goal_return_find_open_lowest_cost(uint16_t cell_count)
{
    uint16_t best_cost = NAV_GOAL_RETURN_COST_INF;
    int16_t best_index = -1;
    for (uint16_t i = 0u; i < cell_count; ++i) {
                                         ...
                                         uint16_t *path_count_out)
{
    ...
    while (cursor >= 0) {
        if (reverse_count >= NAV_GOAL_RETURN_MAX_CELLS) {
            return false;
        }
                                            ...
                                            bool allow_back_entry)
{
    for (uint16_t i = 0u; i + 1u < path_count; ++i) {
        ...
        int8_t to_x = 0;
        int8_t to_y = 0;
        goal_return_decode_index(path[i], width, &from_x, &from_y);
        goal_return_decode_index(path[i + 1u], width, &to_x, &to_y);
    ...
    NavGoalReturnEvalResult *result)
{
    ...
    goal_return_workspace.cost[source_index] = 0u;

    bool budget_blocked_any_transition = false;
    while (true) {
        const int16_t current_index_signed = goal_return_find_open_lowest_cost(cell_count);
        if (current_index_signed < 0) {
            break;
        }

        const uint16_t current_index = (uint16_t)current_index_signed;
        if (current_index == target_index) {
            break;
        }
        goal_return_workspace.closed[current_index] = 1u;
        ...
        for (uint8_t dir_value = 0u; dir_value < 4u; ++dir_value) {
            ...
            if (goal_return_workspace.closed[next_index] != 0u) {
                continue;
            }

            const uint16_t next_unknown_cells =
                (uint16_t)goal_return_workspace.unknown_cells[current_index]
                + (info.target_unvisited ? 1u : 0u);
            const uint16_t next_unknown_edges =
                (uint16_t)goal_return_workspace.unknown_edges[current_index]
            ...
            const uint32_t candidate_cost =
                (uint32_t)goal_return_workspace.cost[current_index] + step_cost;
            if (candidate_cost >= NAV_GOAL_RETURN_COST_INF
                || candidate_cost >= goal_return_workspace.cost[next_index]) {
                continue;
            }

            goal_return_workspace.cost[next_index] = (uint16_t)candidate_cost;
            goal_return_workspace.parent[next_index] = (int16_t)current_index;
            goal_return_workspace.parent_dir[next_index] = dir_value;
            goal_return_workspace.unknown_cells[next_index] =
                goal_return_clamp_u8(next_unknown_cells);
            goal_return_workspace.unknown_edges[next_index] =
                goal_return_clamp_u8(next_unknown_edges);
        }
    }

    if (goal_return_workspace.cost[target_index] == NAV_GOAL_RETURN_COST_INF) {
    ...
    goal_return_workspace.debug.optimistic_return_cost =
        goal_return_workspace.cost[target_index];
    goal_return_workspace.debug.unknown_cells_on_path =
        goal_return_workspace.unknown_cells[target_index];
    goal_return_workspace.debug.unknown_edges_on_path =
        goal_return_workspace.unknown_edges[target_index];

    uint16_t path_count = 0u;
    if (!goal_return_reconstruct_path(source_index,
                                      target_index,
    ...
    goal_return_workspace.debug.score_improvement =
        ...
        - (int32_t)goal_return_workspace.debug.optimistic_return_cost;

    if ((uint32_t)goal_return_workspace.debug.optimistic_return_cost
        + (uint32_t)config->score_margin
        < (uint32_t)config->safe_return_cost) {
